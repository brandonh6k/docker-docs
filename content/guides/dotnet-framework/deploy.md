---
title: Test your .NET Framework deployment
linkTitle: Test your deployment
weight: 50
keywords: deploy, .net framework, local, development, windows containers
description: Learn how to deploy your application
aliases:
  - /language/dotnet-framework/deploy/
  - /guides/language/dotnet-framework/deploy/
---

## Prerequisites

- Complete all the previous sections of this guide, starting with [Containerize
  a .NET Framework application](containerize.md).
- [Turn on Kubernetes](/manuals/desktop/features/kubernetes.md#install-and-turn-on-kubernetes) in Docker
  Desktop.
- Enable Windows containers in Docker Desktop and ensure you have Windows nodes available for Kubernetes.
...
Note: .NET Framework applications require Windows nodes in Kubernetes.

## Create a Kubernetes YAML file

In your `docker-dotnet-framework-sample` directory, create a file named
`docker-dotnet-framework-kubernetes.yaml`. Open the file in an IDE or text editor and add
the following contents. Replace `DOCKER_USERNAME/REPO_NAME` with your Docker
username and the name of the repository that you created in [Configure CI/CD for
your .NET Framework application](configure-ci-cd.md).
        ...
        - name: wait-for-db
          ...
          command:
            [
              "cmd",
              "/c",
              'timeout /t 30 & echo "waiting for db"',
            ]
      nodeSelector:
...
- A Deployment, describing a scalable group of identical pods. In this case,
  ...
  .NET Framework application](configure-ci-cd.md).
...
1. In a terminal, navigate to the `docker-dotnet-framework-sample` directory
   and deploy your application to Kubernetes.

   ```console
   $ kubectl apply -f docker-dotnet-framework-kubernetes.yaml
   ```
