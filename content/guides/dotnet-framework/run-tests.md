---
title: Run .NET Framework tests in a container
linkTitle: Run your tests
weight: 30
keywords: .NET Framework, test, windows containers
description: Learn how to run your .NET Framework tests in a container.
aliases:
  - /language/dotnet-framework/run-tests/
  - /guides/language/dotnet-framework/run-tests/
---

## Prerequisites

Complete all the previous sections of this guide, starting with [Containerize a .NET Framework application](containerize.md).
...
The sample application already has a test project inside the `tests` directory. When developing locally, you can use Compose to run your tests.

Run the following command in the `docker-dotnet-framework-sample` directory to run the tests inside a container.
...
Passed!  - Failed:     0, Passed:     1, Skipped:     0, Total:     1, Duration: < 1 ms - /source/tests/bin/Debug/net48/tests.dll (net48)
```
...
FROM --platform=$BUILDPLATFORM mcr.microsoft.com/dotnet/framework/sdk:4.8.1 AS build
ARG TARGETARCH
COPY . /source
WORKDIR /source/src
...
FROM mcr.microsoft.com/dotnet/framework/sdk:4.8.1 AS development
COPY . /source
WORKDIR /source/src
...
#11 27.91   myWebApp -> /source/src/bin/Debug/net48/myWebApp.dll
#11 28.47   tests -> /source/tests/bin/Debug/net48/tests.dll
#11 28.49 Test run for /source/tests/bin/Debug/net48/tests.dll (.NETFramework,Version=v4.8.1)
...
#11 32.07
#11 32.08 Passed!  - Failed:     0, Passed:     1, Skipped:     0, Total:     1, Duration: < 1 ms - /source/tests/bin/Debug/net48/tests.dll (net48)
#11 DONE 32.2s
```
